def numberToWords(numbers):

    liczba1 = list(str(numbers))
    liczba1.reverse()
    ...
    cyfry = (
    'zero', 'jeden', 'dwa', 'trzy', 'cztery', 'pięć', 'sześć', 'siedem', 'osiem', 'dziewięć', 'dziesięć', 'jedenaście',
        ...
        if nr in {0, 3, 6, 9}:
            if nr == 3:  # sprawdza czy tysiące
                if len(liczba1) > 4 and int(liczba1[nr + 1]) == 1:
                    ...
                    slownie.insert(0, tysiace[2])
            elif nr == 6:  # sprawdza czy miliony
                if len(liczba1) > 7 and int(liczba1[nr + 1]) == 1:
                    slownie.insert(0, miliony[2])
                elif 1 < int(cyfra) < 5:
                    slownie.insert(0, miliony[1])
                elif int(cyfra) != 1:
                    slownie.insert(0, miliony[2])
            elif nr == 9:  # sprawdza czy miliardy
                if len(liczba1) > 10 and int(liczba1[nr + 1]) == 1:
            ...
            if int(cyfra) == 1 or int(
                    cyfra) == 0:  # dla cyfry 0 i 1 nie pisze nic (dla jedynki już wypisano przy poprzednim przebiegu)
                continue
